  ...
  try {
    ...
    if (array_key_exists("id",$_GET)) {
      ...
      if($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        try {
          // create db query
          // ADD AUTH TO QUERY
          $query = $readDB->prepare('SELECT id, lektiko_room, room_code from room where id = :id');
    ...
    elseif(empty($_GET)) {
      ...
      // if request is a GET e.g. get rooms
      if($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        try {
          // ADD AUTH TO QUERY
          // create db query
          $query = $readDB->prepare('SELECT id, lektiko_room, room_code from room');
          $query->execute();
          ...
          // bundle rooms and rows returned into an array to return in the json data
          $returnData = array();
          $returnData['rows_returned'] = $rowCount;
          $returnData['rooms'] = $roomArray;
        ...
        // if error with sql query return a json error
          ...
          $response->setHttpStatusCode(500);
          $response->setSuccess(false);
          $response->addMessage("Failed to get rooms");
          $response->send();
          exit;
        }
      }
